...
// on sendDocument/sendPhoto calls with a file attached. A dedicated
// keep-alive, IPv4-forced agent is the standard fix for this on Render -
// it avoids the flaky IPv6 route and reuses one warm connection instead of
// opening (and sometimes losing) a new one per request.
const telegramAgent = new https.Agent({keepAlive:true, family:4, timeout:30000});
...
if(TELEGRAM_BOT_TOKEN){
  ...
  bot = new Telegraf(TELEGRAM_BOT_TOKEN, {telegram:{agent: telegramAgent}});
